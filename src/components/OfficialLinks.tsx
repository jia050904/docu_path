...
export function OfficialLinks({procedure}:{procedure:Procedure}){
  ...
  </dl>{procedure.officialApplicationUrl&&<a className="primary-link-button" href={procedure.officialApplicationUrl} target="_blank" rel="noopener noreferrer" onClick={()=>track('official_link_click',{procedure_id:procedure.id,service_name:procedure.title,link_type:'application'})}>{applicationLabel(procedure)} <ExternalLink size={16}/></a>}{stay&&<p className="application-path">Hi Korea → 민원신청 → 전자민원 → 등록외국인의 체류기간연장허가</p>}{sources.length>0&&<div className="named-source-links">{sources.map(url=><a key={url} href={url} target="_blank" rel="noopener noreferrer">{sourceName(url)} <ExternalLink size={13}/></a>)}</div>}</section>;
}
