...
export function DocumentChecklist({procedure,answers}:{procedure:Procedure;answers:Record<string,string>}){
  ...
  const steps=procedure.steps.filter(step=>!step.condition||step.condition.values.includes(answers[step.condition.questionId]));
  const doneDocuments=selected.filter(id=>documents.some(document=>document.id===id)).length;
  const doneSteps=(stepChecks[procedure.id]||[]).filter(id=>steps.some(step=>step.id===id)).length;
  ...
  return <section className="detail-section">
    ...
    <ProgressBar done={doneDocuments+doneSteps} total={documents.length+steps.length}/>
